...
function [New_image, New_N] = construct_skeleton (A,B,N,img_A,img_B,img_N, Depth_image)
...
% % % % colorbar();
% % % % F1=getframe(f1);
% % % % imwrite(F1.cdata,'ori2.jpg');
% % % % imwrite(img_A,'ori1.jpg');
% % % % m = imread('ori1.jpg');
% % % % imcontour(m)
...
r = [60,60,30,30, 30,30,30,30,30,30,30,30,30,30,30,30,30,30,30,30];
        ...
        for n = max(1, round(min(yv))):min(first, round(max(yv)))
            ...
            if in==1 && data_check(n,m) == 0
                ...
                New_image(n, m, 3) = ori_img_N(new_n, new_m, 3);
                 if Depth_image(new_n, new_m)>=d_a-100 && Depth_image(new_n, new_m)<=d_b+150
                    ori_img_N(new_n, new_m, 1) = aa;
                    ori_img_N(new_n, new_m, 2) = bb;
                    ori_img_N(new_n, new_m, 3) = cc;
                end
                data_check(n,m) = 1;
            end
        end
