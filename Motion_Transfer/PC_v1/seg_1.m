...
addpath(utilpath);
fnum = 200;
filename='Dataset_v1/Gaurav_3'; % File name for video and also data file
...
for i=1:fnum+1
    ...
    if any(metaData_Depth.IsPositionTracked)~=0
        ...
        Depth_data{start} = All_depth{i};
        
        [In_xyz,flippedDepthImage1] = depthToPointCloud(All_depth{i},depthDevice);
        xyz_data{start} = In_xyz;
...
save(strcat(filename,'_depth'),'Depth_data');
save(strcat(filename,'_xyz'),'xyz_data');